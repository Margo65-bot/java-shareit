...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(long userId, ItemDto itemDto) {
        ...
        if (requestId != null) {
            item.setRequest(itemRequestStorage.findById(requestId).orElseThrow(() -> new NotFoundException("Запрос с id " + requestId + " не найден")));
        }
        item.setUser(UserDtoMapper.mapToModel(user));

        itemDto = ItemDtoMapper.mapToDto(itemStorage.save(item));
